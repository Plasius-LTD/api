...
identify a different commit from the package and tag. The implementation also
still supplied a long-lived npm write token despite documentation requiring
trusted publishing.
...
5. A read-only hosted job repeats exact-main and exact-CI checks, validates and
   packs with lifecycle scripts disabled, creates the SBOM, and uploads both as
   digest-bound direct artifacts.
6. The hosted `production` job verifies exact artifact IDs, digests, package
   identity, safe members, npm distribution tag, and SHA-512 registry
...
repository `api`, workflow `cd.yml`, environment `production`, and action
